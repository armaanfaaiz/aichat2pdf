...
export function ViewSwitcher({ activeMode, onModeChange }: ViewSwitcherProps) {
  ...
  return (
    <div className="view-switcher-bar no-print w-full bg-white/85 dark:bg-slate-900/85 backdrop-blur-xl rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm p-2 sm:p-2.5 mb-5 transition-colors">
      <div className="flex items-center justify-between px-2 mb-2">
        ...
        <span className="text-[11px] text-indigo-600 dark:text-indigo-400 font-semibold hidden sm:inline-flex items-center gap-1">
          <span>Instant live preview</span>
          <span className="text-slate-300 dark:text-slate-600">&bull;</span>
          <span className="text-slate-500 dark:text-slate-400 font-normal">Click to switch layout</span>
        </span>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 p-1 bg-slate-100/70 dark:bg-slate-800/80 rounded-xl">
        {views.map((v) => {
          const Icon = v.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg font-bold text-xs transition-all duration-200 ${
                isActive
                  ? 'bg-white dark:bg-slate-900 text-indigo-950 dark:text-indigo-300 shadow-md shadow-slate-900/5 ring-1 ring-slate-900/5 dark:ring-slate-700/50 transform scale-[1.02]'
            ...
            >
              <Icon className={`w-4 h-4 transition-colors ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'}`} />
